...
%eating function for class healthy human
%agt=healthy human object
%cn - current agent number
%eaten = 1 if healthy human finds food, =0 otherwise
...
%SUMMARY OF healthy human EAT RULE
%healthy human detects food level in its 1km x 1km square of the environment
%if food> 1, healthy human will consume food
%otherwise healthy human food level decremented by 1.
...
if pfood>=1                             %if food exists at this location
    ...
    eaten=1;                             %healthy human has eaten - set flag to one
else
    agt.food=cfood-1;                   %decrease agent food by one unit
    eaten=0;                            %flag tells healthy human to migrate
end
